...
export function SolutionsTeaser() {
  return <section className="section"><div className="wrap"><Reveal className="section-head" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", flexWrap: "wrap", gap: 20 }}><div><span className="eyebrow">What we do</span><h2>Steam kitchens, end to end.</h2></div><Link className="btn btn-glass glass" to="/services">
            Explore solutions <ArrowUpRight size={15} /></Link></Reveal><div className="teaser-grid" style={{ marginTop: 44 }}>{CARDS.map(({ icon: Icon, k, h, p }, i) => <Reveal key={k} delay={i * 80}><Link to="/services" className="teaser-card glass"><div className="disc glass" aria-hidden="true"><Icon size={24} strokeWidth={1.6} /></div><span className="k">{k}</span><h3>{h}</h3><p>{p}</p></Link></Reveal>)}</div></div></section>;
}
